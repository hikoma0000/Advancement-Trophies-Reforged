...
public class TrophyItemRenderer {
    ...
    public static void renderFirstPersonIcon(LivingEntity entity, ItemStack itemStack, ItemDisplayContext displayContext, PoseStack poseStack, MultiBufferSource buffer, int packedLight) {
        ...
        if (nbt == null || !nbt.contains(NBTKeys.ICON)) {
            return;
        }
        ...
        float scaleX = columnLength(matrix, 0);
        float scaleY = columnLength(matrix, 1);
        float scaleZ = columnLength(matrix, 2);

        Vector3f tipPos = new Vector3f(matrix.m30(), matrix.m31(), matrix.m32());
        ...
        setIdentityRotation(matrix, scaleX, scaleY, scaleZ);
        matrix.m30(FOLLOW.pos.x);
        matrix.m31(FOLLOW.pos.y);
        matrix.m32(FOLLOW.pos.z);
        ...
        if (!CitadelCompat.tryRender(iconStack, ItemDisplayContext.FIXED, poseStack, buffer, packedLight, OverlayTexture.NO_OVERLAY)
                && !AlexsMobsCompat.tryRender(iconStack, ItemDisplayContext.FIXED, poseStack, buffer, packedLight, OverlayTexture.NO_OVERLAY)) {
    ...
    private static float columnLength(Matrix4f matrix, int column) {
        return switch (column) {
            case 0 -> (float) Math.sqrt(matrix.m00() * matrix.m00() + matrix.m10() * matrix.m10() + matrix.m20() * matrix.m20());
            case 1 -> (float) Math.sqrt(matrix.m01() * matrix.m01() + matrix.m11() * matrix.m11() + matrix.m21() * matrix.m21());
            default -> (float) Math.sqrt(matrix.m02() * matrix.m02() + matrix.m12() * matrix.m12() + matrix.m22() * matrix.m22());
        };
    }
